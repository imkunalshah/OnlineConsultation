...
import com.kunal.onlineconsultation.Sendbird.FileUtils;
import com.kunal.onlineconsultation.Sendbird.MediaPlayerActivity;
import com.kunal.onlineconsultation.Sendbird.PhotoViewerActivity;
import com.kunal.onlineconsultation.Sendbird.PreferenceUtils;
import com.kunal.onlineconsultation.Sendbird.TextUtils;
import com.kunal.onlineconsultation.Sendbird.UrlPreviewInfo;
import com.kunal.onlineconsultation.Sendbird.WebUtils;

import com.kunal.onlineconsultation.Sendbird.groupchannel.CreateGroupChannelActivity;
import com.kunal.onlineconsultation.Sendbird.groupchannel.GroupChatAdapter;
import com.kunal.onlineconsultation.Sendbird.groupchannel.InviteMemberActivity;
import com.kunal.onlineconsultation.Sendbird.groupchannel.MemberListActivity;
...
public class ChatActivity extends AppCompatActivity {
    public static String EXTRA_GROUP_CHANNEL_URL = "GROUP_CHANNEL_URL";
    ...
    private static String STATE_CHANNEL_URL = "STATE_CHANNEL_URL";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        SendBirdSyncManager.setup(getApplicationContext(), FirebaseAuth.getInstance().getCurrentUser().getUid(), new CompletionHandler() {
            @Override
        ...
        if (savedInstanceState != null) {
            mChannelUrl = getIntent().getExtras().get("url").toString();
        } else {
...
//        ((GroupChannelActivity) getApplicationContext()).setOnBackPressedListener(new GroupChannelActivity.onBackPressedListener() {
//            @Override
...
//        });
    ...
    public void onResume() {
        ...
        mChatAdapter.setContext(getApplicationContext()); // Glide bug fix (java.lang.IllegalArgumentException: You cannot start a load for a destroyed activity)`
                    ...
                    MessageCollection.create(channelUrl, mMessageFilter, mLastRead, new MessageCollectionCreateHandler() {
                        ...
                        public void onResult(MessageCollection messageCollection, SendBirdException e) {
                            if (e == null) {
                                ...
                                //Toast.makeText(getApplicationContext(), getString(R.string.get_channel_failed), Toast.LENGTH_SHORT).show();
    ...
    private void updateActionBarTitle() {
        ...
        if (getApplicationContext() != null) {
            //((GroupChannelActivity) getApplicationContext()).setActionBarTitle(title);
        }
    }
        ...
        final UserMessage pendingMessage = mChannel.sendUserMessage(text, new BaseChannel.SendUserMessageHandler() {
            ...
            public void onSent(UserMessage userMessage, SendBirdException e) {
                ...
                if (e != null) {
                    // Error!
                    Log.e(LOG_TAG, e.toString());
                    Toast.makeText(getApplicationContext(), getString(R.string.send_message_error, e.getCode(), e.getMessage()), Toast.LENGTH_SHORT).show();
                    return;
                }
            }
        });
    ...
    private void sendFileWithThumbnail(Uri uri) {
        ...
        Hashtable<String, Object> info = FileUtils.getFileInfo(getApplicationContext(), uri);

        if (info == null || info.isEmpty()) {
            Toast.makeText(getApplicationContext(), getString(R.string.wrong_file_info), Toast.LENGTH_LONG).show();
            return;
        }
        final String name;
        ...
        if (path == null || path.equals("")) {
            Toast.makeText(getApplicationContext(), getString(R.string.wrong_file_path), Toast.LENGTH_LONG).show();
        } else {
            ...
            BaseChannel.SendFileMessageWithProgressHandler progressHandler = new BaseChannel.SendFileMessageWithProgressHandler() {
                ...
                public void onSent(FileMessage fileMessage, SendBirdException e) {
                    ...
                    mMessageCollection.fetchAllNextMessages(null);
                    if (e != null) {
                        Log.d("MyTag", "onSent: " + getApplicationContext());
                        if (getApplicationContext() != null) {
                            Toast.makeText(getApplicationContext(), getString(R.string.sendbird_error_with_code, e.getCode(), e.getMessage()), Toast.LENGTH_SHORT).show();
                        }
                    }
                }
            };
